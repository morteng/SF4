...
class TestBotRoutes:
    ...
    def test_update_bot(self, admin_auth_client, db):
        # Create a bot first
        ...
        # Update the bot
        response = admin_auth_client.post(url_for('admin_bot.update', id=bot.id), data={
            'name': 'Updated Name',
    ...
    def test_delete_bot(self, admin_auth_client, db):
        # Create a bot first
        ...
        # Delete the bot
        response = admin_auth_client.post(url_for('admin_bot.delete', id=bot.id), follow_redirects=True)
        assert response.status_code == 200
        assert Bot.query.get(bot.id) is None

    def test_run_bot(self, mocker, admin_auth_client, db):
        ...
        # Run the bot
        response = admin_auth_client.post(url_for('admin_bot.run', id=bot.id), follow_redirects=True)
        assert response.status_code == 200
