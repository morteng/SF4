...
def db(app):
    ...
    with app.app_context():
        ...
        # Create all tables
        _db.create_all()
        print("Database created successfully")  # Debug statement
...
def init_admin_user(session):
    ...
    session.commit()
    print(f"Admin user created with token: {admin_user.auth_token}")  # Debug statement
    return admin_user
...
def test_bot(session):
    ...
    session.add(bot)
    session.commit()
    print(f"Bot created with ID: {bot.bot_id}")  # Debug statement
    return bot
