...
def test_update_organization(logged_in_admin, test_organization, db_session):
    with logged_in_admin.application.app_context():
        # Fetch CSRF token from the form page
        response = logged_in_admin.get(url_for('admin.organization.edit', id=test_organization.id))  # Updated endpoint name
        csrf_token = extract_csrf_token(response.data)
        ...
        response = logged_in_admin.post(url_for('admin.organization.edit', id=test_organization.id), data=updated_data)  # Updated endpoint name
...
def test_update_organization_with_invalid_form_data(logged_in_admin, test_organization, db_session):
    with logged_in_admin.application.app_context():
        ...
        response = logged_in_admin.post(url_for('admin.organization.edit', id=test_organization.id), data=invalid_data)  # Updated endpoint name
...
def test_update_organization_with_invalid_id(logged_in_admin):
    update_response = logged_in_admin.get(url_for('admin.organization.edit', id=9999))  # Updated endpoint name
